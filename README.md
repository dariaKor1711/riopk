# riopk
# **Программное средство бронирования оздоровительных услуг частной клиники**

Цель проекта
Создание удобной, безопасной и масштабируемой информационной системы, позволяющей клиентам частной клиники записываться на оздоровительные процедуры онлайн, а персоналу — эффективно управлять расписанием, загрузкой специалистов и взаимодействием с пациентами.

Основные возможности:
- Онлайн-бронирование услуг
	- Выбор даты, времени и специалиста
	- Просмотр доступных слотов
	- Подтверждение и отмена записи
- Управление расписанием
	- Администратор может добавлять, редактировать и удалять слоты
	- Автоматическая блокировка занятых временных интервалов
- Каталог оздоровительных процедур
	- Описание услуг, цены, длительность
	- Фильтрация по типу, специалисту, доступности
- Профили пользователей
	- Регистрация и авторизация
 	- Роли: клиент, администратор, доктор
	- История записей и уведомления
- Интеграция с внешними сервисами
	- Отправка SMS/email-напоминаний
- Аналитика и отчётность
	- Статистика по посещаемости, популярности услуг
	- Загрузка специалистов

Ссылки на репозитории сервера и клиента

---

## **Содержание**

1. [Архитектура](#Архитектура)
	1. [C4-модель](#C4-модель)
	2. [Схема данных](#Схема_данных)
2. [Функциональные возможности](#Функциональные_возможности)
	1. [Диаграмма вариантов использования(#Диаграмма_вариантов_использования)]
	2. [User-flow диаграммы](#User-flow_диаграммы)
3. [Детали реализации](#Детали_реализации)
	1. [UML-диаграммы](#UML-диаграммы)
Use-case диаграмма
<img width="1069" height="1022" alt="Use case drawio" src="https://github.com/user-attachments/assets/543f887d-61e9-4440-8fc7-ae9e95426cb2" />

Диаграмма последовательности
<img width="1585" height="970" alt="Диаграмма последовательности" src="https://github.com/user-attachments/assets/fd19f437-d5a7-4a72-bd83-452ca3d42f2a" />

Диаграмма деятельности
<img width="908" height="1361" alt="диаграмма деятельности (1)" src="https://github.com/user-attachments/assets/6359f379-a2f9-4e48-98e8-6a2bffea3716" />

Диаграмма компонентов
<img width="2962" height="1095" alt="BPMN(as-is)-комп drawio" src="https://github.com/user-attachments/assets/cbdfa488-1101-4d46-aa94-60d4be27e113" />

Диаграмма развертывания
<img width="1501" height="479" alt="BPMN(as-is)-разверт drawio" src="https://github.com/user-attachments/assets/804760a7-f758-48e4-9dbb-d5b910973740" />

	3. [Спецификация API](#Спецификация_API)
	4. [Безопасность](#Безопасность)
	5. [Оценка качества кода](#Оценка_качества_кода)
4. [Тестирование](#Тестирование)
	1. [Unit-тесты](#Unit-тесты)
	2. [Интеграционные тесты](#Интеграционные_тесты)
5. [Установка и  запуск](#installation)
	1. [Манифесты для сборки docker образов](#Манифесты_для_сборки_docker_образов)
	2. [Манифесты для развертывания k8s кластера](#Манифесты_для_развертывания_k8s_кластера)
6. [Лицензия](#Лицензия)
7. [Контакты](#Контакты)

---
## **Архитектура**

### C4-модель
Для визуализации архитектуры программного решения применяется методология C4-модели, которая позволяет представить систему на четырёх уровнях детализации. Каждый уровень раскрывает архитектуру более глубоко: начиная с общего представления (контекст), далее — распределение по контейнерам, затем — описание компонентов внутри контейнеров, и, наконец, — структура классов. Контекстный уровень демонстрирует, как система взаимодействует с внешними пользователями и другими сервисами.

 <img width="791" height="461" alt="РиОПКЛР11 drawio (1)" src="https://github.com/user-attachments/assets/b7ad30d1-3bbc-4db6-865c-d02dee53c0e5" />

 На данном этапе архитектура программного средства рассматривается через призму контейнеров – ключевых технических блоков, обеспечивающих работу всей системы. Элементы, определенные на контекстной диаграмме, уточняются и группируются по функциональному назначению: пользовательский интерфейс, серверная логика, база данных и внешние интеграции.
В рамках проекта бронирования оздоровительных услуг частной клиники контейнерный уровень включает:
– Веб-клиент, реализованный на React, предоставляющий интерфейс для клиентов, администраторов и врачей;
– Серверное приложение на Node.js, обрабатывающее бизнес-логику, запросы и взаимодействие с базой данных;
– Хранилище данных на PostgreSQL, содержащее информацию о пользователях, услугах, расписании и бронированиях;


 <img width="881" height="841" alt="РиОПКЛР12 drawio (1)" src="https://github.com/user-attachments/assets/3b448893-53c4-416f-9324-8231cc042bb0" />

 На компонентном уровне архитектура системы рассматривается более детально – здесь раскрываются внутренние элементы каждого контейнера, определенного ранее. Этот уровень позволяет описать ключевые программные компоненты, такие как контроллеры, сервисы, модели и репозитории, а также их взаимодействие между собой.
В контексте программного средства бронирования оздоровительных услуг частной клиники компонентный уровень включает:
– На фронтенде (React): компоненты пользовательского интерфейса, формы записи, календарь доступных слотов, личный кабинет;
– На серверной части (Node.js): REST-контроллеры для обработки запросов, сервисы с бизнес-логикой, репозитории для доступа к данным, модели сущностей;
– Связь с базой данных (PostgreSQL): через ORM или SQL-запросы, обеспечивающие хранение информации о пользователях, услугах, расписании и бронированиях.
 
<img width="1571" height="671" alt="РиОПКЛР13 drawio" src="https://github.com/user-attachments/assets/178fded7-2aab-406b-b9e1-ea76b3a2aa9f" />

Самый детализированный уровень архитектурного описания — кодовый. Он раскрывает внутреннюю структуру компонентов, показанных на предыдущем уровне, включая классы, интерфейсы и их связи.

<img width="4112" height="3484" alt="image" src="https://github.com/user-attachments/assets/7a1f6949-a296-4a98-8700-d2dab57968d8" />

### Схема данных
<img width="1280" height="1191" alt="Untitled (1)" src="https://github.com/user-attachments/assets/a8fd80ab-a313-44b2-9c28-71c591727e1c" />


---

## **Функциональные возможности**

### Диаграмма вариантов использования

Диаграмма вариантов использования и ее описание

### User-flow диаграммы

<img width="4290" height="2363" alt="Untitled" src="https://github.com/user-attachments/assets/a2606d7e-5050-422e-b4d2-5e125bf3f6e4" />
<img width="4870" height="2363" alt="2" src="https://github.com/user-attachments/assets/4ad4f4ac-1f2a-4110-b0bb-672e40cc7e12" />
<img width="5902" height="2494" alt="3" src="https://github.com/user-attachments/assets/e6307afb-eb4f-4581-90ef-3db2f4033e3c" />

Описание переходов между части ПС для всех ролей из диаграммы ВИ (название ролей должны совпадать с тем, что указано на c4-модели и диаграмме вариантов использования)

---

## **Детали реализации**

### UML-диаграммы

Представить все UML-диаграммы , которые позволят более точно понять структуру и детали реализации ПС

### Спецификация API

Представить описание реализованных функциональных возможностей ПС с использованием Open API (можно представить либо полный файл спецификации, либо ссылку на него)

### Безопасность

Описать подходы, использованные для обеспечения безопасности, включая описание процессов аутентификации и авторизации с примерами кода из репозитория сервера

### Оценка качества кода

Используя показатели качества и метрики кода, оценить его качество

---

## **Тестирование**

### Unit-тесты

Представить код тестов для пяти методов и его пояснение

### Интеграционные тесты

Представить код тестов и его пояснение

---

## **Установка и  запуск**
1 Для успешной установки и запуска программного приложения необходимо наличие следующих компонентов:
– операционная система семейства Windows 10 / macOS / Linux;
– Node.js версии 18 и выше;
– npm или yarn;
– Docker Engine + Docker Compose;
– сервер базы данных PostgreSQL версии 14 и выше (если используется без Docker);
– среда разработки Visual Studio Code или WebStorm;
– NestJS CLI;
– React.js (в составе frontend-проекта).
2 Для установки разработанного программного приложения необходимо выполнить следующие шаги:
– установить Node.js и менеджер пакетов (npm или yarn);
– установить NestJS CLI командой: npm i -g @nestjs/cli;
– установить Docker и Docker Compose;
– клонировать репозиторий проекта (frontend, backend);
– настроить файл окружения .env для backend-сервера;
– настроить файл .env.local для frontend-проекта;
– выполнить сборку и запуск backend через Docker Compose или локально;
– выполнить сборку и запуск frontend;
– убедиться, что соединение backend и базы данных установлено; 
– открыть веб-портал в браузере по локальному адресу.
3 В поставляемый конечному пользователю дистрибутив входят следующие элементы:
– скрипт создания пустой базы данных (SQL-файл);
– конфигурационный файл .env.back для backend;
– конфигурационный файл .env.local.front для frontend;
– исходный код backend-части на NestJS;
– исходный код frontend-части на React.js;
– docker-compose.yml для автоматического запуска PostgreSQL и backend;
– README-файл с базовой информацией;
– seed-данные для создания ролей (админ, клиент, специалист).
4 Размещение файлов в системных каталогах не требуется. Docker-контейнеры создадут необходимые внутренние директории автоматически.
5 Восстановление базы данных. Для восстановления базы данных программного приложения из резервной копии необходимо выполнить следующие шаги:
5.1 Запуск PostgreSQL. При использовании Docker:

docker-compose up -d postgres

При использовании локальной установки PostgreSQL – подключиться через терминал:

sudo -u postgres psql

5.2 Создание пустой базы данных

CREATE DATABASE booking;

После успешного выполнения команда выдаст сообщение: 

CREATE DATABASE.

5.3 Создание пользователя (при необходимости)

CREATE USER booking_user WITH PASSWORD 'password123';

5.4 Назначение прав пользователю

GRANT ALL PRIVILEGES ON DATABASE booking TO booking_user;

5.5 Выполнение SQL-скрипта. Если база данных запускается через Docker:

docker exec -i <postgres-container-medica> psql -U booking_user booking < ./db/init.sql

Если PostgreSQL установлен локально:

psql -U booking_user booking < ./db/init.sql

5.6 Результат после успешного выполнения скрипта будут созданы:
– таблицы пользователей;
	– таблицы ролей (клиент, специалист, администратор);
	– таблицы бронирований;
	– таблицы расписаний;
	– базовые записи для корректной работы портала.
6 Настройка дистрибутива
6.1 Настройка backend-части. Файл окружения .env расположен в директории backend. Он может иметь структуру:

DATABASE_HOST=localhost
DATABASE_PORT=5432
DATABASE_USER=booking_user
DATABASE_PASSWORD=password123
DATABASE_NAME=booking
PORT=5000
JWT_SECRET=secret_key_123

Необходимо:
– заменить логин и пароль на фактические значения;
	– указать корректный порт локального PostgreSQL или Docker-контейнера;
	– при необходимости изменить секрет для JWT-токенов.
6.2 Настройка frontend-части. Файл .env.local:

NEXT_PUBLIC_API_URL=http://localhost:5000/api

Здесь следует указать URL backend-сервера.
7 Проверка работоспособности веб-портала
7.1 Запуск backend, используя Docker:

docker-compose up -d backend

Или локально:

cd backend
npm install
npm run start:dev

7.2 Запуск frontend:

cd frontend
npm install
npm run dev

7.3 Проверка соединения с сервером
– открыть браузер;
	– перейти по адресу: http://localhost:3000

7.4 Ожидаемый результат при успешной установке, откроется главная страница веб-портала, а пользователю будут доступны:
– кабинет клиента (просмотр записей);
	– кабинет специалиста (управление расписанием и клиентами);
	– админ-панель (записи, специалисты, клиенты, аналитика).


### Манифесты для сборки docker образов

Представить весь код манифестов или ссылки на файлы с ними (при необходимости снабдить комментариями)

### Манифесты для развертывания k8s кластера

Представить весь код манифестов или ссылки на файлы с ними (при необходимости снабдить комментариями)

---

## **Лицензия**

Этот проект лицензирован по лицензии MIT - подробности представлены в файле [[License.md|LICENSE.md]]

---

## **Контакты**

Автор: email

